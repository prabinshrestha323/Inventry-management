  ...
  <div class="row content">
    <div class="col-sm-3 sidenav">
      ...
      <ul class="nav nav-pills nav-stacked">
        ...
        <li class="active"><a href="#Customers"><span class="glyphicon glyphicon-user"></span> Customers</a></li>
    ...
    <div class="col-sm-9">
      <h4><small>MANAGE CUSTOMERS</small></h4>
      <hr>

      	<div class="container-fluid">
          <div class="row">
            <div class="col-sm-3">
              <a href="#" class="btn btn-primary btn-xl" onclick="anc()">Add new customer</a>
            </div>
            <div class="col-sm-9">
            </div>
          </div>
...
if (isset($_GET['q'])) {
  ...
  if (isset($_GET['page']) && isset($_GET['per-page'])) {
                 ...
                 $result = mysqli_query($con,"SELECT COUNT(code) FROM tbl_customer WHERE code LIKE '%$q%' OR name LIKE '%$q%' OR address LIKE '%$q%'"); 
                 ...
                 if ($total_pages < $_GET['page']) {
                  ...
                  if ($total_records == 0) {
                    # code...
                    $_SESSION['empty-row'] = "0 customer to display. Add customer.";
                  }else{
                    header("Location:?page=1&per-page=5&q=$q&search");
                  }
                 }
          ...
          $customer = mysqli_query($con,"SELECT * FROM tbl_customer WHERE code LIKE '%$q%' OR name LIKE '%$q%' OR address LIKE '%$q%' ORDER BY id DESC LIMIT $start_from, $perpage"); 
}else{
          ...
          if (isset($_GET['page']) && isset($_GET['per-page'])) {
                 ...
                 $result = mysqli_query($con,"SELECT COUNT(code) FROM tbl_customer"); 
                  $row = mysqli_fetch_row($result); 
                  $total_records = $row[0]; 
                  $total_pages = ceil($total_records / $perpage);
          ...
          $customer = mysqli_query($con,"SELECT * FROM tbl_customer ORDER BY id DESC LIMIT $start_from, $perpage"); 
}
           ...
           ?>
              ...
              <?php
                ...
                <tr class="text-info">
                  ...
                  <td>
                  <p class="text-center">
                    <a href="#" class="btn btn-xs btn-info" data-page="edit" data-name="<?php echo $row['id']; ?>" title="Click this if you want to edit all the information for this customer."><span class="glyphicon glyphicon-edit"></span></a>
                    <a href="#" class="btn btn-xs btn-danger" data-page="delete" data-name="<?php echo $row['id']; ?>" title="Click this if you want to delete this customer."><span class="glyphicon glyphicon-trash"></span></a>
                  </p>
                  </td>
                </tr>
                <?php }
